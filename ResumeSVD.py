...
def resumSVD(word_embedding, dataset_name = "", heuristic_rate = 0, testSize = 100, numberSentencesOP = 0,
           kd = False, ordreClus= "avant"):
...
def main():
    ...
    elif longDocument == "resumSVD":
        launcherSVDCONC(dataset_name, size_output)
    else:
        if scoringP == True:
            scoring(dataset_name)
        else:    
            resumSVD(word_embedding_path, dataset_name = dataset_name, heuristic_rate = heuristic_rate, testSize = size_output, numberSentencesOP = nb_sentences, kd = kdpara, ordreClus = ordreClus)
            scoring(dataset_name)
